...
/** Mesmo visual do MissoesDrawer real, mas com dados estáticos passados por
 * prop em vez de buscar em /api/missoes (que exige sessão de verdade). */
export function MissoesDrawerDemo({ missoes, descontosPendentes }: { missoes: MissaoComProgresso[]; descontosPendentes: MissaoDesconto[] }) {
  const [open, setOpen] = useState(false)
  const ref = useRef<HTMLDivElement>(null)
  const emAndamento = missoes.filter((m) => !m.concluida).length
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        {emAndamento > 0 && (
          <span
            className="absolute top-1 right-1 bg-gradient-to-br from-rose-500 to-orange-500 text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center leading-none"
            suppressHydrationWarning
          >
