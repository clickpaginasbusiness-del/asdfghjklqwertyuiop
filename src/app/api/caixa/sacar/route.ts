...
import { solicitarSaqueCaixa } from '@/lib/caixa'
...
export async function POST(request: NextRequest) {
  ...
  const resultado = await solicitarSaqueCaixa(admin, prestadora.id, valor, pixChave)

  if (!resultado.ok) {
    return NextResponse.json({ error: resultado.error }, { status: 400 })
  }

  return NextResponse.json({ ok: true, mensagem: 'Seu saque será processado em breve.' })
}
