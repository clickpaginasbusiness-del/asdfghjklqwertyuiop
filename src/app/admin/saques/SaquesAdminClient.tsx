...
export default function SaquesAdminClient({ saques }: { saques: SaqueRow[] }) {
  ...
  async function confirmarPagamento(id: string) {
    setConfirmandoId(id)
    try {
      const res = await fetch(`/api/admin/saques/${id}/confirmar`, { method: 'POST' })
  ...
  function mensagemComprovante(nome: string, valor: number) {
    return `Olá ${nome}! Seu saque de ${formatCurrency(valor)} foi processado! Segue o comprovante: [espaço para colar]`
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto p-6 lg:p-8 space-y-6">
        <Card className="border-amber-200">
          <CardHeader>
            <CardTitle>Saques solicitados ({pendentes.length})</CardTitle>
          </CardHeader>
                  ...
                  return (
                    <div key={s.id} className="p-5 space-y-3">
                      <div className="flex items-start justify-between gap-3 flex-wrap">
                        <div>
                          <p className="font-semibold text-gray-900">{s.parceiraNome}</p>
                      ...
                      {jaConfirmado && (
                        <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3 space-y-2">
                          ...
                          {s.whatsappTelefone ? (
                            <a
                              href={buildWhatsappUrl(s.whatsappTelefone, mensagemComprovante(s.parceiraNome, s.valor))}
                              target="_blank"
                            ...
                            <p className="text-xs text-amber-600">Sem telefone de WhatsApp registrado pra essa parceira.</p>
                          )}
                        </div>
                      )}
                    </div>
                ...
                {pagos.map((s) => (
                  <div key={s.id} className="flex items-center justify-between gap-3 px-5 py-3 flex-wrap">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-gray-900">{s.parceiraNome} — {formatCurrency(s.valor)}</p>
